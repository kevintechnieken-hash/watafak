...
public class AdminCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "kick" -> {
                if (args.length < 2) { sender.sendMessage("Gebruik: /admin kick <player> [reason]"); return true; }
                Player t = Bukkit.getPlayer(args[1]);
                if (t == null) { sender.sendMessage("Niet online."); return true; }
            ...
            case "ban" -> {
                if (args.length < 2) { sender.sendMessage("Gebruik: /admin ban <player> [reason]"); return true; }
                String name = args[1];
            ...
            case "tp" -> {
                if (args.length < 3) { sender.sendMessage("Gebruik: /admin tp <player> <target>"); return true; }
                Player a = Bukkit.getPlayer(args[1]);
                Player b = Bukkit.getPlayer(args[2]);
                if (a == null || b == null) { sender.sendMessage("Speler niet gevonden."); return true; }
                a.teleport(b.getLocation());
            }
            case "resetstats" -> {
                if (args.length < 2) { sender.sendMessage("Gebruik: /admin resetstats <player>"); return true; }
                Player t = Bukkit.getPlayer(args[1]);
                if (t == null) { sender.sendMessage("Niet online."); return true; }
                plugin.getStatsManager().resetStats(t.getUniqueId());
                sender.sendMessage("Stats gereset.");
            }
            case "send" -> {
                if (!(sender instanceof Player p)) { sender.sendMessage("Players only"); return true; }
                if (args.length < 3) { sender.sendMessage("Gebruik: /admin send <player|all> <server>"); return true; }
                ...
                if (target.equalsIgnoreCase("all")) {
                    ...
                    if (pl == null) { sender.sendMessage("Niet online."); return true; }
                    BungeeUtil.connect(pl, server, plugin);
                }
                sender.sendMessage("Verstuurd naar server " + server);
            }
            case "reload" -> {
                plugin.getConfigManager().reloadAll();
                sender.sendMessage("Configs herladen.");
            }
